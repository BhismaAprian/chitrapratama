...
<div class="relative h-72 w-full bg-black flex justify-center align-items-center"
    ...
    <div class="text-center">
        <div class="text-2xl font-bold text-white mb-4">
            Product Knowladge - Fact Sheet
        </div>
        <div class="text-gray-300 text-xs">
            Home / Product Knowladge / Fact Sheet / {{ isset($_GET['keyword']) && $_GET['keyword'] != '' ? 'Search /
            '.$_GET['keyword'] :
            'Newest' }}
        </div>
    </div>
    <div class="w-10 h-10 flex absolute bottom-0">
        <a href="#fact-sheet" class="m-auto">
...
{{-- Start Section Card --}}
<section id="fact-sheet" class="mx-auto sm:px-32 px-4 sm:py-14 py-3">
    <div class="grid lg:grid-cols-3 grid-cols-2 mx-auto lg:gap-8 gap-4">
        @forelse($lembar_faktas as $lembar_fakta)
        <div
            ...
            <a href="{{ url('lembar_fakta/'.$lembar_fakta->slug) }}" class="relative overflow-hidden">
                <img class="rounded-t-lg" src="{{ asset('storage/'. $lembar_fakta->image) }}" alt="" />
            </a>
            <div class="sm:p-7 p-4">
                <a href="{{ url('lembar_fakta/'.$lembar_fakta->slug) }}">
                    <h5
