...
<div class="relative h-72 w-full bg-black flex justify-center align-items-center"
    ...
    <div class="text-center">
        ...
        <div class="text-gray-300 text-xs">
            Home / Prees Release / {{ isset($_GET['keyword']) && $_GET['keyword'] != '' ? 'Search / '.$_GET['keyword'] :
            'Newest' }}
        </div>
    </div>
    <div class="w-10 h-10 flex absolute bottom-0">
        <a href="#siaran-pers" class="m-auto">
...
{{-- Start Section Card --}}
<section id="siaran-pers" class="mx-auto sm:px-32 px-4 sm:py-14 py-3">
    <div class="grid lg:grid-cols-3 grid-cols-2 mx-auto lg:gap-8 gap-4">
        @forelse($siarans as $siaran)
        <div
            class="transform scale-100 hover:scale-105 transition-transform duration-300 ease-in-out my-4 bg-white border border-gray-200 rounded-xl shadow dark:bg-gray-800 dark:border-gray-700 group self-start">
            <a href="{{ url('siaran-pers/'. $siaran->slug) }}" class="relative overflow-hidden">
                <img class="rounded-t-xl" src="{{ asset('storage/'. $siaran->image) }}" alt="" />
            </a>
